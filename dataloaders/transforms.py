...
from .kspace_subsample import RandomMaskFunc
...
class ReconstructionTransform(object):
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        # fig = plt.figure()
        # plt.axis('off')
        # plt.imshow(torch.log10(torch.abs(complex_abs(kspace))+1), cmap = 'gray')
        # fig.savefig('./test_images/pdfs_kspace.png', bbox_inches='tight', pad_inches=0)           
        
        # kspace = torch.fft.fftshift(kspace)
        
        # kspace = torch.fft.fftshift(kspace, dim=(1, 2))
        # print('kspace after fftshift',kspace.shape, kspace.dtype)
        ...
        if self.mask_func:
            ...
            # fig.savefig('./test_images/mask.png', bbox_inches='tight', pad_inches=0)
            
        else:
            masked_kspace = kspace

        # # masked_kspace = torch.fft.ifftshift(masked_kspace)
        # image = torch.fft.ifft2(masked_kspace)
        # # image = torch.fft.ifftshift(image)
        # print('sampled_image', image.shape, image.dtype)      
        
        image = fastmri.ifft2c(masked_kspace)
        # print('image after iff2c',image.shape, image.dtype)
        ...
        image = complex_center_crop(image, crop_size)
        # print('image after center crop', image.shape, image.dtype)
        # absolute value
        image = fastmri.complex_abs(image)
        # print('image after complex_abs', image.shape, 'min', image.min(), 'max', image.max(), 'mean', image.mean(), 'std', image.std())
        # image = torch.abs(image)
        # print('image after abs', image.shape, 'min', image.min(), 'max', image.max(), 'mean', image.mean(), 'std', image.std())
        
        # print(image[150:170, 150:170])
        # image *= 10000
        ...
        # normalize input
        # print('image before normalize', image.shape, 'min', image.min(), 'max', image.max(), 'mean', image.mean(), 'std', image.std())
        image, mean, std = normalize_instance(image, eps=1e-11)
        # print('image after norm', image.shape, 'min', image.min(), 'max', image.max(), 'mean', image.mean(), 'std', image.std())
        ## image.max(), the result is from image after normalization
        image = image.clamp(-6, 6)

        # normalize target
        if target is not None:
            target = to_tensor(target)
            # print('target before center crop', target.shape, 'min', target.min(), 'max', target.max(), 'mean', target.mean(), 'std', target.std())
            target = center_crop(target, crop_size)
            # print('target before normalize', target.shape, 'min', target.min(), 'max', target.max(), 'mean', target.mean(), 'std', target.std())
            target = normalize(target, mean, std, eps=1e-11)
            # print('target after normalize', target.shape, 'min', target.min(), 'max', target.max(), 'mean', target.mean(), 'std', target.std())
            target = target.clamp(-6, 6)
        else:
            target = torch.Tensor([0])
            
            # target *= 10000
        # print('image', image.shape, 'min', image.min(), 'max', image.max(), 'mean', image.mean(), 'std', image.std())
        # print('target', target.shape, 'min', target.min(), 'max', target.max(), 'mean', target.mean(), 'std', target.std(), 'mean', mean, 'std', std)
...
def build_transforms(mode = 'train', MASKTYPE='Random', CENTER_FRACTIONS=[0.08], ACCELERATIONS=[4]):
    
    # print('Center fractions:', CENTER_FRACTIONS, 'Accelerations:', ACCELERATIONS)

    if mode == 'train':
        # mask = create_mask_for_mask_type(MASKTYPE, CENTER_FRACTIONS, ACCELERATIONS,)
        # mask = MaskFunc_Cartesian(CENTER_FRACTIONS, ACCELERATIONS) 
        mask = RandomMaskFunc(CENTER_FRACTIONS, ACCELERATIONS)
